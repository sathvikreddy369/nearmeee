...
//           // Use the fixed markMessagesAsRead function
...
//   // Effect to handle initiating a chat from a vendor's page
//   useEffect(() => {
//     const initChat = async () => {
//       if (!urlVendorId || !currentUser || loadingConversations) return;
...
//     initChat();
//   }, [urlVendorId, currentUser, conversations, loadingConversations, addToast]);
...
function MessagesPage() {
  ...
  useEffect(() => {
    const initChat = async () => {
      // CRITICAL FIX: Add 'selectedConversation' to the condition to prevent re-running 
      // every time the 'conversations' list is updated by the real-time listener.
    ...
    initChat();
    // Added selectedConversation to ensure the effect reruns if selection is cleared (unlikely) 
    // but mainly to stop the effect from rerunning repeatedly when 'conversations' updates.
